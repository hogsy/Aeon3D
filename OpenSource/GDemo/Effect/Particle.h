...
/*
	...
	destroyed by returning GE_TRUE to keep the particle and GE_FALSE to destroy the particle.
...
/*
	...
	callback should return GE_TRUE if the particle is to remain alive, GE_FALSE if the
	particle should be destroyed this frame.  If GE_FALSE is returned, the particle will
	be destroyed in the current frame without being rendered.
*/
typedef	geBoolean	(PARTICLE_CALLINGCONVENTION *ParticleCallBack)(Particle_System *, Particle *, geFloat);
...
//Particle_System * PARTICLE_CALLINGCONVENTION Particle_SystemCreate(GE_World *World, int milliseconds);
...
void PARTICLE_CALLINGCONVENTION Particle_SystemAddParticle(
	...
	geBitmap *		Texture,		// Texture to draw with
	const GE_LVertex	*	Vert,			// Vertex at which and how to draw the sprite
	const geVec3d			*AnchorPoint,	// anchor point
	geFloat					Time,			// How long this particle should live, in seconds
	const geVec3d *			Velocity,		// Velocity vector to use to move the particle
	float					Scale,			// scale of particle art
	const geVec3d			*Gravity );		// gravity which will act on the particle
