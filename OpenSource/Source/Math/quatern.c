...
#	define geQuaternion_Assert \
		if ( geQuaternion_MaximalAssertionMode ) assert
...

{
	...
	Q->W = ( ( Q1L.W * Q2L.W ) - ( Q1L.X * Q2L.X ) - ( Q1L.Y * Q2L.Y ) - ( Q1L.Z * Q2L.Z ) );

	Q->X = ( ( Q1L.W * Q2L.X ) + ( Q1L.X * Q2L.W ) + ( Q1L.Y * Q2L.Z ) - ( Q1L.Z * Q2L.Y ) );

	Q->Y = ( ( Q1L.W * Q2L.Y ) - ( Q1L.X * Q2L.Z ) + ( Q1L.Y * Q2L.W ) + ( Q1L.Z * Q2L.X ) );

	Q->Z = ( ( Q1L.W * Q2L.Z ) + ( Q1L.X * Q2L.Y ) - ( Q1L.Y * Q2L.X ) + ( Q1L.Z * Q2L.W ) );
...
// returns GE_TRUE if Q is a unit geQuaternion.  GE_FALSE otherwise.
{
	...
	magnitude = ( Q->W * Q->W ) + ( Q->X * Q->X ) + ( Q->Y * Q->Y ) + ( Q->Z * Q->Z );
...
// normalizes Q to be a unit geQuaternion
{
	...
	magnitude = ( geFloat ) sqrt( ( Q->W * Q->W ) + ( Q->X * Q->X ) + ( Q->Y * Q->Y ) + ( Q->Z * Q->Z ) );
...
geBoolean GENESISCC geQuaternion_Compare( geQuaternion *Q1, geQuaternion *Q2, geFloat Tolerance )
{
	...
	if (                                                                                                                                                                               // they are the same but with opposite signs
	        ( ( fabs( Q1->X + Q2->X ) <= Tolerance ) && ( fabs( Q1->Y + Q2->Y ) <= Tolerance ) && ( fabs( Q1->Z + Q2->Z ) <= Tolerance ) && ( fabs( Q1->W + Q2->W ) <= Tolerance ) ) ||// they are the same with same signs
	        ( ( fabs( Q1->X - Q2->X ) <= Tolerance ) && ( fabs( Q1->Y - Q2->Y ) <= Tolerance ) && ( fabs( Q1->Z - Q2->Z ) <= Tolerance ) && ( fabs( Q1->W - Q2->W ) <= Tolerance ) ) )
		return GE_TRUE;
	else
		return GE_FALSE;
}
