...
static void Server_SetupGenVSI(Server_Server *Server);
static geBoolean Server_ManageBots(Server_Server *Server);
...
static geBoolean FillBufferWithClientInfo(Server_Server *Server, Buffer_Data *Buffer)
{
	...
	for (i=0; i< NETMGR_MAX_CLIENTS; i++, Client++)
	{
		...
		Buffer_FillByte(Buffer, (char)i);
		Buffer_FillString(Buffer, Client->Name);
...
geBoolean Server_SendClientCurrentWorldData(Server_Server *Server, Server_Client *Client)
{
	...
	for (i=0; i< GAMEMGR_MAX_TEXTURE_INDEX; i++)
	{
		...
		Buffer_FillSLong(&Buffer, i);
		Buffer_FillString(&Buffer, TextureIndex->FileName);
...
void Server_ValidateClient(Server_Server *Server, Server_Client *Client)
{
	...
	Player = Server_CreatePlayer(Server, Client->Name);
...
geBoolean Server_SpawnWorld(Server_Server *Server)
{
	...
	for (i=0; i < Server->NumClassSpawns; i++)
	{
		...
		while (1)
		{
			...
			Player = Server_CreatePlayer(Server, CSpawn->Name);
...
//=====================================================================================
static GPlayer *Server_GetNextPlayerInRadius(void *Server, GPlayer *Player, GPlayer *Start, const char *ClassName, float Radius)
{
	while (1)
	{
		geVec3d		Vect;

		Start = Server_GetNextPlayer(Server, Start, ClassName);
...
//	Server_Error
//=====================================================================================
static void Server_Error(Server_Server *Server, const char *ErrorStr, ...)
{
	va_list		ArgPtr;
