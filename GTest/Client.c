		...
		while (Buffer.Pos < Buffer.Size)
		{
			...
			switch(Type)
			{
				...
				case NETMGR_MSG_ACTOR_INDEX:
				{
					...
					Buffer_GetString(&Buffer, ActorName);
				...
				case NETMGR_MSG_MOTION_INDEX:
				{
					...
					Buffer_GetString(&Buffer, MotionName);
				...
				case NETMGR_MSG_BONE_INDEX:
				{
					...
					Buffer_GetString(&Buffer, BoneName);
				...
				case NETMGR_MSG_TEXTURE_INDEX:
				{
					...
					Buffer_GetString(&Buffer, FileName);
					Buffer_GetString(&Buffer, AFileName);
				...
				case NETMGR_MSG_CLIENT_NAME:
				{
					...
					Buffer_GetString(&Buffer, Client->ClientInfo[ClientIndex].Name);
					break;
				}
...
static void UpdatePlayers(Client_Client *Client, float Time)
{
	...
	#if 1
		Player->Control = Client_GetProcIndex(Client, Player->ControlIndex);
	...
	if (!(PlayerIndexFlags & (1<<15)) )
	{
		...
		if (Client->Mode == ClientMode_Proxy)
		{
			...
			if (pPlayer->ControlIndex == 0xffff)
				...
				pPlayer->Control = Client->ProcIndex[pPlayer->ControlIndex];
			...
			{
				pPlayer->Trigger = Client->ProcIndex[pPlayer->TriggerIndex];
			}
		}
...
//=====================================================================================
//=====================================================================================
static float Client_GetTime( void *data )
{
	Client_Client *Client = ( Client_Client * ) data;
	assert( Client_IsValid( Client ) == GE_TRUE );
...
//	Client_GetWorld
//=====================================================================================
geWorld *Client_GetWorld( void *data )
{
	Client_Client *Client = ( Client_Client * ) data;
	assert( Client_IsValid( Client ) == GE_TRUE );
...
//=====================================================================================
static GenVSI_CMove *Client_GetClientMove( void *data, GenVSI_CHandle ClientHandle )
{
	Client_Client *Client = ( Client_Client * ) data;
	assert( Client_IsValid( Client ) == GE_TRUE );
...
//	Client_ProcIndex
//=====================================================================================
static void Client_ProcIndex( void *data, uint16 Index, void *Proc )
{
	Client_Client *Client = ( Client_Client * ) data;
	assert( Client_IsValid( Client ) == GE_TRUE );
...
//=====================================================================================
static geBoolean Client_MovePlayerModel( void *data, void *playerData, const geXForm3d *DestXForm )
{
	geBoolean CanMove;
	int32     i;
	geWorld  *World;
...
//=====================================================================================
static void *Client_SpawnTempPlayer( void *data, const char *ClassName )
{
	int32 i;
...
//=====================================================================================
void Client_DestroyTempPlayer( void *data, void *playerData )
{
	Client_Client *Client = ( Client_Client * ) data;
	assert( Client_IsValid( Client ) == GE_TRUE );
